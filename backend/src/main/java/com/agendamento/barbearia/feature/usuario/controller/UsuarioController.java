...
public class UsuarioController {
    ...
    public ResponseEntity<?> getLoggedUser(Authentication authentication) {
        // Se não houver ninguém logado, devolve 401 Unauthorized
        if (authentication == null || !authentication.isAuthenticated() || "anonymousUser".equals(authentication.getPrincipal())) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("logged", false));
        }

        String email = null;
        // Identifica se veio do Google ou do Login Normal
        ...
        response.put("role", user.getRole()); // CLIENTE, BARBEIRO ou ADMIN

        // Se for um Barbeiro, mandamos também o ID profissional dele para a Dashboard!
